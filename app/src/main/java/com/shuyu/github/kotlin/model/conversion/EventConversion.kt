...
object EventConversion {
    ...
    fun eventToEventUIModel(event: Event): EventUIModel {
        ...
        when (event.type) {
            ...
            "PushEvent" -> {
                ...
                des = ""
                var descSpan: String? = ""
                val count = event.payload?.commits?.size ?: 0
                val maxLines = 4
    ...
    private fun eventAction(event: Event, eventUIModel: EventUIModel) {
        ...
        when (event.type) {
            ...
            "ReleaseEvent" -> {
                ...
                eventUIModel.releaseUrl =
                    url ?: ""/*CommonUtils.launchWebView(context, repositoryName, url);*/
            }
    ...
    fun notificationToEventUIModel(context: Context, notification: Notification): EventUIModel {
        ...
        eventUIModel.des =
            notification.reason + " " + "${context.getString(R.string.notifyType)}：$type，${
                context.getString(R.string.notifyStatus)
            }：$status"
